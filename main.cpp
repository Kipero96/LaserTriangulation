...
static void zhangsuenThinningIteration(Mat img, int iter)
{
	/*
	...
	*/
	...
	for (int i = 1; i < img.rows - 1; i++)
	{
		for (int j = 1; j < (img.cols / 2); j++)
		{
			...
			int transitionsWhiteToBlack = (p2 == 0 && p3 == 1) + (p3 == 0 && p4 == 1) +
										  (p4 == 0 && p5 == 1) + (p5 == 0 && p6 == 1) +
										  (p6 == 0 && p7 == 1) + (p7 == 0 && p8 == 1) +
										  (p8 == 0 && p9 == 1) + (p9 == 0 && p2 == 1);
			int blackPixelNeighbours = p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9;
			int m1 = iter == 0 ? (p2 * p4 * p6) : (p2 * p4 * p8); //find out what it means
			int m2 = iter == 0 ? (p4 * p6 * p8) : (p2 * p6 * p8); //find out what it means

			if (transitionsWhiteToBlack == 1 && (blackPixelNeighbours >= 2 && blackPixelNeighbours <= 6) && m1 == 0 && m2 == 0)
...
static void guohallThinningIteration(Mat img, int iter)
{
	for (int i = 1; i < img.rows - 1; i++)
	{
